...
- Для paid-сценария уже начат первый шаг без платежного шлюза: backend отдает каталог тарифов и умеет сразу активировать/продлевать подписку через purchase endpoint.
- Для paid-сценария теперь добавлен первый реальный платежный контур через `Platega SBP`: checkout создается на backend, а подтверждение идет через webhook и только потом активирует подписку.
