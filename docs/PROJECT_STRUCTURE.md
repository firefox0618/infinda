...
- `tests` — заготовка под интеграционные, e2e и другие общие тесты уровня системы.
...
- В `apps/api` уже есть app-level тесты по доменам, но корневой `tests/` пока остается только заготовкой под интеграционный и e2e-слой.
